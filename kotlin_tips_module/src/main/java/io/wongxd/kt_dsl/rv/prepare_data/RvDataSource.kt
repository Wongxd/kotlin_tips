...
open class RvDataSource<T> {
    protected open val dataStorage = RvDataStorage<T>()

    private val fetchingState = FetchingState()
    private val invalid = AtomicBoolean(false)
    private var pagingListDiffer = RvDataListDiffer<T>()
    ...
    private fun onLoadResult(data: List<T>?) {
        if (data != null) {
            if (data.isEmpty()) {
                if (dataStorage.itemSize() == 0) {
                    changeState(FetchingState.DONE_FETCHING)
                } else {
                    changeState(FetchingState.NO_MORE_DATA)
                }
            } else {
     ...
     *  [FetchingState.DONE_FETCHING],[FetchingState.NO_MORE_DATA],
     *  [FetchingState.READY_TO_FETCH]
     */
